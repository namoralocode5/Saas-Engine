...
def verify_gumroad_license(product_permalink, license_key):
  ...
  try:
    ...
    data = res.json()
    return data.get("success", False) and not data.get("purchase", {}).get(
        "refunded", False
    )
  except Exception:
    return False
...
# Initialize session state (Domyślne wartości dla wprowadzania danych)
...
with st.expander(
    "🔑 PRO License Activation / Gumroad Key", expanded=not is_pro_init
):
  user_key = st.text_input(
      "Enter your Gumroad License Key:", type="password", key="user_key_input"
  )
  ...
  if is_pro:
    ...
    st.markdown(
        '<a'
        ' href="https://namoralocode.gumroad.com/l/arbitrage-pulse-pro"'
        ' target="_blank" class="buy-btn">💳 Get PRO License on Gumroad</a>',
        unsafe_allow_html=True,
    )
    st.caption("Enter your valid license key purchased on Gumroad.")

# 4. Parameters and Sliders (Poprawione powiązanie parametrów za pomocą key)
with st.expander("⚙️ Portfolio, Fees & Exchange Settings", expanded=False):
  st.number_input("Capital ($)", min_value=100, step=500, key="capital")
  st.slider("Leverage", min_value=1, max_value=5, key="leverage")
  ...
  st.number_input(
      "Est. Total Trading Fee (%) per round-trip",
      min_value=0.0,
      max_value=0.2,
      step=0.01,
      format="%.2f",
...
if st.button("🔍 SCAN MARKET NOW"):
  with st.spinner("Fetching funding rates from exchanges..."):
    ...
    for ex in st.session_state.selected_exchanges:
      data = cached_fetch_exchange_data(
          ex, min_volume_usd=st.session_state.min_vol
      )
      if data:
        all_ex_data[ex] = data
    ...
    for asset in all_assets:
      if asset in EXCLUDED_SYMBOLS:
        continue
      available_exchanges = [
          ex for ex in all_ex_data if asset in all_ex_data[ex]
      ]
      if len(available_exchanges) < 2:
        continue

      min_ex = min(
          available_exchanges, key=lambda x: all_ex_data[x][asset]["rate_pct"]
      )
      max_ex = max(
          available_exchanges, key=lambda x: all_ex_data[x][asset]["rate_pct"]
      )
      ...
      payments_per_day = 3
      gross_annual_apy = (
          spread_pct * payments_per_day * 365 * st.session_state.leverage
      )
      ...
      if net_annual_apy >= 0.0:
        est_profit_year = round(
            (st.session_state.capital * net_annual_apy) / 100, 2
        )
        results.append({
            "asset": asset,
...
if "scan_results" in st.session_state and st.session_state.scan_results:
  ...
  with col_apy:
    min_apy_filter = st.number_input(
        "📉 Min. Net APY (%)", min_value=0.0, value=0.0, step=5.0
    )
  with col_filter:
  ...
  if search_query:
    filtered_results = [
        item for item in filtered_results if search_query in item["asset"]
    ]

  if min_apy_filter > 0:
    filtered_results = [
        item for item in filtered_results if item["net_apy"] >= min_apy_filter
    ]
  ...
  if only_my_exchanges:
    ...
    filtered_results = [
        ...
        if item["long_ex"] in active_set and item["short_ex"] in active_set
    ]
  ...
  if filtered_results:
    ...
    st.download_button(
        ...
        data=csv_data,
        file_name=(
            f"arbitrage_pulse_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        ),
        mime="text/csv",
    )
    st.write("")
    ...
    for item in filtered_results:
      if is_pro:
        long_display = (
            f'<a href="{item["long_url"]}" target="_blank" style="color:'
            f' #00e676; text-decoration: none; font-weight:'
            f' bold;">{item["long_ex"]} ↗</a>'
        )
        short_display = (
            f'<a href="{item["short_url"]}" target="_blank" style="color:'
            f' #ff5252; text-decoration: none; font-weight:'
            f' bold;">{item["short_ex"]} ↗</a>'
        )
        pro_button_html = ""
        strategy_line = (
            '<div style="margin-top: 4px; font-size: 11px; color: #00e676;'
            f' font-family: monospace;">📋 Copy Plan: LONG {item["long_ex"]} |'
            f' SHORT {item["short_ex"]} | {item["asset"]}/USDT</div>'
        )
      else:
        long_display = (
            '<span style="color: #ffb300; font-weight: bold;">🔒 PRO</span>'
        )
        short_display = (
            '<span style="color: #ffb300; font-weight: bold;">🔒 PRO</span>'
        )
        pro_button_html = (
            '<a'
            ' href="https://namoralocode.gumroad.com/l/arbitrage-pulse-pro"'
            ' target="_blank" class="card-buy-btn">💳 Unlock Exchanges on'
            " Gumroad</a>"
        )
        strategy_line = ""
...
<div style="display: flex; justify-content: space-between; align-items: center; font-size: 11px; color: #8f9cae;">
    ...
    if not is_pro:
      st.warning(
          "🔒 Activate PRO license to unlock exchanges and direct links."
      )
      st.markdown(
          '<a'
          ' href="https://namoralocode.gumroad.com/l/arbitrage-pulse-pro"'
          ' target="_blank" class="buy-btn">💳 Get PRO License & Unlock'
          " Exchanges</a>",
          unsafe_allow_html=True,
      )
  else:
