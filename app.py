...
def verify_license(license_key):
  ...
  # BYPASS DLA SZYBKIEGO TESTU / NAGRANIA (wpisanie "1")
...
with col_badge:
  st.markdown(
      "<span class='badge-official'>OFFICIAL</span>", unsafe_allow_html=True
  )
...
with st.expander("🔑 **PRO License Activation**", expanded=True):
  ...
  if is_pro:
    ...
    st.link_button(
        ...
        "https://namoralocode.gumroad.com/l/arbitrage-pulse-pro",
    )
...
if st.button("🔍 SCAN MARKET NOW") or "scanned" in st.session_state:
  ...
  st.caption(
      f"🕒 Last Update: {datetime.datetime.utcnow().strftime('%H:%M:%S')} UTC"
  )
  ...
  search_query = (
      st.text_input("🔍 Search Asset:", placeholder="np. XRP, HMSTR")
      .strip()
      .upper()
  )
  ...
  filtered = [
      p
      for p in mock_pairs
      if (not search_query or search_query in p["symbol"])
      and p["apy"] >= min_apy
  ]
  ...
  for p in filtered:
    ...
    st.markdown(
        f"### **{p['symbol']}** &nbsp; <span style='background:#065f46;"
        f" color:#a7f3d0; padding:2px 8px;"
        f" border-radius:4px;'>+{p['apy']:.2f}% APY</span>",
        unsafe_allow_html=True,
    )
    ...
    if is_pro:
      with col_l:
        ...
        st.link_button(
            "Trade Long", f"https://www.{p['long'].lower()}.com"
        )
      with col_s:
        st.markdown(f"🔴 **SHORT:** `{p['short']}`")
        st.link_button(
            "Trade Short", f"https://www.{p['short'].lower()}.com"
        )
    else:
      with col_l:
    ...
    st.info(
        f"💵 **Est. Profit (${capital}):** `+${est_profit:.2f} / yr` | Spread:"
        f" +{p['spread']}%"
    )
    st.markdown("---")
